...
async def delete_board_db(db: AsyncSession, board: Board):
...
async def get_all_tasks_by_id_in_board_db(db: AsyncSession, board_id: UUID):
    stmt = select(Task).where(Task.board_id == board_id)
    result = await db.execute(stmt)
    return result.scalars().all()


async def get_task_by_id_in_board_db(db: AsyncSession, board_id: UUID, task_id: UUID):
...
async def delete_task_by_id_in_board_db(
    db: AsyncSession, board_id: UUID, task_id: UUID, current_user_id: UUID
):
    stmt = select(Task).where(Task.id == task_id, Task.board_id == board_id)
    result = await db.execute(stmt)
